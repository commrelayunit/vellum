...
**How to fix it:** if the bridge supports registering additional tools per-session or per-request, register `edit_document` there so it actually forwards through and calls back into Vellum's API — check that framework's own docs for dynamic/passthrough tool support. If it only supports a fixed, hardcoded toolset with no passthrough mechanism, it isn't currently compatible with this feature; plain chat (no editing) still works through it.
